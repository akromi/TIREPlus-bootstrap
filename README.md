...
booking pages as `{{TEKMETRIC_SHOP_ID}}` and into the config global in the
`tekmetric-booking` script block — then `node build.js`, commit, push.
...
reaches a different shop. `test.js` pins the expected id, so **update
